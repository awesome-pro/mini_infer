...
scripts/check_*.py      139 checks (no test framework — plain asserts)
...
for f in check_engine check_memory check_preemption check_metrics check_benchmark \
         check_policies check_visualizations check_torch_runner; do
  python scripts/$f.py
done
```
...
phase 8 real model runner: all 15 checks passed
```
...
## 16. Experiment 8 — a real model in the runtime

...
```
miniserve/engine/      request model, scheduling contract, policies, engine loop
miniserve/memory/      KV block pool, content-addressed prefix cache (BlockPlan)
miniserve/runner/      simulated cost model, and the real torch runner (paged KV)
miniserve/metrics/     TTFT / ITL / TPOT / throughput / KV / fragmentation
miniserve/benchmark/   workloads, driver, sweeps, exports
miniserve/visualizations/  text timelines and charts, matplotlib figures
scripts/check_*.py      155 checks (no test framework, plain asserts)
examples/*.py           seven runnable demos, each showing one idea
...
```bash
pip install -e .        # once, from the repo root; makes `import miniserve` work
...
- `Request.on_prefill` / `on_decode` — `miniserve/engine/request.py`
- The budget loop — `SchedulerBase._build_plan` in `miniserve/engine/policies.py`
- Cost model — `miniserve/runner/simulated_runner.py`
...
from miniserve.memory import PagedBlockManager
...
`miniserve/memory/block_manager.py`.
...
python -m miniserve.cli --requests 120 --sweep rate --values 2,4,8,16,32,64,128,256

# KV pool pressure
python -m miniserve.cli --requests 120 --sweep num_blocks --values 16,32,64,128

# budget effect
python -m miniserve.cli --requests 120 --sweep max_batch_tokens --values 16,32,64,128

# export for a README or plot
python -m miniserve.cli --sweep rate --values 4,16,64 --export /tmp/results
```
...
```bash
python -m miniserve.cli --requests 150 --rate 12 --sweep max_batch_tokens \
...
python -m miniserve.cli --requests 120 --arrival burst --num-blocks 256 \
...
python -m miniserve.cli --sweep prefix_cache --values off,on
...
# CLI
python -m miniserve.cli --help
python -m miniserve.cli --requests 200 --arrival poisson --rate 16
python -m miniserve.cli --policy balanced --prefill-reservation 16 --max-wait-steps 8
python -m miniserve.cli --sweep policy --values decode_first,prefill_first,balanced,static
python -m miniserve.cli --sweep block_size --values 8,16,32,64
python -m miniserve.cli --sweep num_blocks --values 16,32,64,128 --export /tmp/r
```
...
`miniserve/visualizations/plots.py`:
